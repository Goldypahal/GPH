...
| **TASK-DAY1-01** | E2E Intelligence Pipeline | Complete 16-stage end-to-end intelligence chain validation from RTSP ingestion to cryptographic WORM custody seal. | **P0** | **VERIFIED** | Implemented `tests/test_e2e_full_chain.py` with machine-readable results (`artifacts/e2e/e2e_results.json`, `summary.md`). | Hardware RTSP streams | PASS (93/93) | `7e2d875` |
| **TASK-DEMO-01** | Demonstration Automation | Deterministic 12-stage demonstration tooling (`scripts/demo_seed.py`, `scripts/demo_reset.py`, `scripts/run_demo.py`, `demo.bat`, `demo.sh`). | **P1** | **VERIFIED** | Automated live suspect pursuit demo with PTS timing, hotlist matching, spatio-temporal route, physics audit, and WORM evidence seal. | None (Software fixture) | PASS | `7e2d875` |
| **TASK-SEC-02** | Security / Hygiene | Eliminate dev auth bypass in production, enforce production secret validations (Pydantic model validator for SECRET_KEY/MINIO), and remove synthetic fallbacks in telemetry. | **P0** | **VERIFIED** | Enforced 401 on missing credentials in production, added Settings model_validator, measured db_query_latency via perf_counter, eradicated fake GPU/latency fallbacks. | None (Software fix) | PASS (98/98) | Working tree |
