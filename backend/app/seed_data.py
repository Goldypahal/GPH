...
from backend.app.models.orm import Department, Camera, CameraHealth, CameraCredential, VehicleSighting, Watchlist, Alert, AuditLog, User, Case, CaseTimelineEntry, CaseEvidence
...
def seed_database():
    ...
    try:
        ...
        # 2b. Sentinel Live Camera Grid (cam01 - cam30 on 103.250.160.189:8554)
        home_dept_id = dept_map.get("HOME_POLICE", list(dept_map.values())[0])
        for spec in SENTINEL_GRID_SPECS:
            cam_id = spec["id"]
            if cam_id not in cam_code_map:
                rtsp_url = f"rtsp://{sentinel_email_enc}:{sentinel_pass_enc}@{SENTINEL_GATEWAY_HOST}/stream/{cam_id}"
                cam = Camera(
                    department_id=home_dept_id,
                ...
                cred = CameraCredential(
                    ...
                    encrypted_password=SENTINEL_PASSWORD,
                    auth_type="BASIC",
                    port=8554
                )
                db.add(cred)
