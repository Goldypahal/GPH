...
function renderVideoWall() {
  ...
  if (currentCameras.length === 0) {
    container.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding:40px;">No cameras found for the selected filter.</div>`;
    return;
  }
...
function focusVideoWallCamera(logicalCode) {
  ...
  // Find camera in allLoadedCameras and put it as first in list
